\subsection{What is programmable cryptography?}
...
\begin{itemize}
\item
  \emph{Digital signatures}. RSA and ElGamal are examples of digital
  ...
  be a group signature scheme,\footnote{https://en.wikipedia.org/wiki/Group\_signature}
  which allows one member of a group to sign a message on behalf of the
  group.
\item
  \emph{Confidential computing}. For example, consider
  Yao\textquotesingle s millionaire problem,\footnote{https://en.wikipedia.org/wiki/Yao\%27s\_Millionaires\%27\_problem}
...
\subsection{Ideas in programmable cryptography}
...
\subsubsection{2PC: two-party computation}

In a \emph{two-party computation (2PC)}, two people want to jointly
...
generalizes to \emph{multi-party computation (MPC)}, which is one of the
main classes of programmable cryptography.

\subsubsection{SNARKs: proofs of general statements}
...
\subsubsection{FHE: fully homomorphic encryption}
...
not want the translating service to know anything about your text at
all.

In \emph{fully homomorphic encryption (FHE)}, one person encrypts some
...
encrypt your text \(\operatorname{Enc}(x)\) and send it to your FHE
machine translation server. The server will faithfully translate it into
another language and give you \(\operatorname{Enc}(y)\), where \(y\) is
the translation of \(x\). You can then decrypt and obtain \(y\), knowing
that the server cannot extract anything meaningful from
\(\operatorname{Enc}(x)\) without your secret key.

\subsubsection{ORAM: Oblivious RAM}
...
\emph{Oblivious RAM (ORAM)} protects against exactly this sort of
attack. Oblivious RAM is an algorithm you use to ``scramble'' your
...
\subsection{Programmable cryptography in the world}
