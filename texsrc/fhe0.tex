\subsection{FHE and leveled FHE}\label{fhe-intro}
...
\begin{enumerate}
\item
  Alice encrypts \(x\) and sends Bob \(\operatorname{Enc}(x)\).
\item
  Bob then ``applies \(f\) to the ciphertext'' and obtains
  \(\operatorname{Enc}\left( f(x) \right)\), sending it to Alice.
\item
  Alice decrypts \(\operatorname{Enc}\left( f(x) \right)\) to learn
  \(f(x)\).
\end{enumerate}
...
({[}lwe-crypto{]}) is more complicated. But it still has this
...
({[}lwe{]}) and how to build cryptography on top of it
({[}lwe-crypto{]}) before we get back to leveled FHE.
