...
Recall that the map
\([ \bullet ]:{\mathbb{F}}_{q} \rightarrow E\) is linear,
meaning that
\([ a + b] = [ a] + [ b]\), and
\([ na] = n[ a]\). But as written we cannot do
...
\[\operatorname{pair}([ m],[ n]) = \operatorname{pair}([ m'],[ n'])\]
will be true whenever \(mn = m'n'\), because both sides will equal
\(mn\operatorname{pair}([ 1],[ 1])\). So this
...
Given \([ x]\), \(\left[ x^{2} \right]\),
\(\left[ x^{3} \right]\), and \([ y]\), Victor
verifies that:

\begin{itemize}
\item
  \(\operatorname{pair}(\left[ x^{2} \right],[ 1]) = \operatorname{pair}([ x],[ x])\)
\item
  \(\operatorname{pair}(\left[ x^{3} \right],[ 1]) = \operatorname{pair}(\left[ x^{2} \right],[ x])\)
\item
