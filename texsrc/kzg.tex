...
globally known.  Now consider an input \(z \in \mathbb{F}_{p}\); Victor wants to know the
...
the verification equation is that \[\begin{aligned}
 & \text{ pair }\left( \left\lbrack Q(s) \right\rbrack,\left\lbrack (s - 1)(s - 2)\ldots(s - 9) \right\rbrack \right) \\
 & = \text{ pair }\left( \left\lbrack P(s) \right\rbrack - 100\left\lbrack s^{2} \right\rbrack,\lbrack 1\rbrack \right).
\end{aligned}\]
...
\(\text{Com }\left( P_{1} \right)\),
\(\text{Com }\left( P_{2} \right)\), and \(\text{Com }(H)\) at
\(\lambda\).
...
if Peggy has sent Victor \(\text{Com }\left( P_{1} \right)\) through
\(\text{Com }\left( P_{6} \right)\), and wants to show that
