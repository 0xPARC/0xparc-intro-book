...
Yao's garbled circuits is one of the most well-known 2PC protocols. The
protocol is quite clever, and optimized variants of the protocol are
being implemented and used today.\footnote{https://github.com/privacy-scaling-explorations/mpz/tree/dev/crates/mpz-garble}
...
\item
  \(f\) is a binary circuit, which takes in \(m + n\) bits, and runs
...
\begin{itemize}
\item
  Alice constructs a \emph{garbled circuit} that takes in the value
  \(b\) (whatever it is) and spits out \(f(a,b)\). A \emph{garbled
  circuit}, roughly speaking, is an ``encrypted'' circuit that takes
  encrypted input and creates encrypted output.
\item
  An \emph{oblivious transfer} is a protocol where Alice has two
...
\item
  ...
  password he is asking for. This is where we will use the ``oblivious
  transfer.''
\end{enumerate}

We now flesh out this outline, starting with garbled circuits.

\subsubsection{Garbled gates}

Our garbled circuits are going to be built out of \emph{garbled gates}.
...
are four possible inputs to the gate: \((0,0),(0,1),(1,0),(1,1)\). For
each of those four inputs \((x,y)\), there is a secret password
\(P_{x,y}\). The gate \(G\) will only reveal its value \(G(x,y)\) if you
...
\(\operatorname{Enc}\) and publish the following table:

\begin{center}
\begin{tabular}{|c|c|}
\hline
\((0,0)\) & \(\operatorname{Enc}_{P_{0,0}}( G(0,0))\) \\
\hline
\((0,1)\) & \(\operatorname{Enc}_{P_{0,1}}( G(0,1))\) \\
\hline
\((1,0)\) & \(\operatorname{Enc}_{P_{1,0}}( G(1,0) )\) \\
\hline
\((1,1)\) & \(\operatorname{Enc}_{P_{1,1}}( G(1,1))\) \\
\hline
\end{tabular}
\end{center}
...
\[\operatorname{Enc}_{P_{x,y}} ( G(x,y) ),\] decrypt it, and
learn \(G(x,y)\).

But if you do not know the password \(P_{x,y}\), assuming
\(\operatorname{Enc}\) is a suitably secure encryption scheme, you will
not learn anything about the value \(G_{x,y}\) from its encryption.

\subsubsection{Chaining garbled gates}

The next step is to combine a bunch of garbled gates into a circuit. We
will need to make two changes to the protocol.
...
\begin{tabular}{|c|c|}
\hline
  \(\operatorname{hash}(P_{0}^{\text{left}},P_{0}^{\text{right}})\) &
  \(\operatorname{Enc}_{P_{0}^{\text{left}},P_{0}^{\text{right}}}( P_{0}^{\text{out}})\) \\
  \hline
  \(\operatorname{hash}(P_{0}^{\text{left}},P_{1}^{\text{right}})\) &
  \(\operatorname{Enc}_{P_{0}^{\text{left}},P_{1}^{\text{right}}}( P_{0}^{\text{out}})\) \\
  \hline
  \(\operatorname{hash}(P_{1}^{\text{left}},P_{0}^{\text{right}})\) &
  \(\operatorname{Enc}_{P_{1}^{\text{left}},P_{0}^{\text{right}}}( P_{0}^{\text{out}})\) \\
  \hline
  \(\operatorname{hash}(P_{1}^{\text{left}},P_{1}^{\text{right}})\) &
  \(\operatorname{Enc}_{P_{1}^{\text{left}},P_{1}^{\text{right}}}( P_{1}^{\text{out}} )\) \\
  \hline
  \end{tabular}
\end{center}


\subsubsection{How Bob uses one gate}
...
\item
  ...
  \(\operatorname{hash}(P_{0}^{\text{left}},P_{1}^{\text{right}})\), and
...
\subsubsection{How the circuit ends}
...
\subsubsection{How the circuit starts}
