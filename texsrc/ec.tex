...
\begin{itemize}
\item
  In Section \ref{ec} we will define \emph{elliptic curves} and
  describe one standard elliptic curve \(E\), the \emph{BN254 curve},
  that will be used in these notes.
\item
  In Section \ref{discretelog} we describe the
  \emph{discrete logarithm assumption} (Assumption \ref{ddh}),
  which we need to make to provide security to our protocols. As an
  example, in Section \ref{eddsa} we describe how
  Assumption \ref{ddh} can be used to construct a signature scheme,
  namely EdDSA.\footnote{https://en.wikipedia.org/wiki/EdDSA}
\item
  The EdDSA idea will later grow up to be the KZG commitment scheme in
  {[}kzg{]}.
\end{itemize}
...
best-known example is RSA,\footnote{https://en.wikipedia.org/wiki/RSA\_(cryptosystem)}
...
(\hyperref[discretelog]{{[}discretelog{]}}) on elliptic curves. But
before we get to the problem, we need to introduce some of the math
behind elliptic curves.

An \emph{elliptic curve} is a set of points with a group operation. The
...
The BN254 specification fixes a specific\footnote{If you must know, the
 values in the specification are given exactly by \[\begin{aligned}
...
blog post\footnote{https://hackmd.io/@jpw/bn254} about the properties of
this curve). The name BN stands for Barreto-Naehrig, two mathematicians
who proposed a family of such curves in 2006.\footnote{https://link.springer.com/content/pdf/10.1007/11693383\_22.pdf}
...
So each point on \(E( {\mathbb{F}}_{p} ))\) is an ordered pair
\((X,Y) \in {\mathbb{F}}_{p}^{2}\) satisfying Equation \ref{bn254eqn}. Okay,
actually, that is a white lie: Conventionally, there is one additional
...
\begin{theorem}
BN254 has prime order: Let \(E\) be the BN254 curve. The number of
points in \(E( {\mathbb{F}}_{p} )\), including the point at
infinity \(O\), is a prime \(q \approx 2^{254}\).
\end{theorem}

\begin{definition}
This prime $q \approx 2^{254}$ is affectionately called the \emph{Baby Jubjub prime}
(a reference to 
\emph{The Hunting of the Snark}\footnote{https://en.wikipedia.org/wiki/The\_Hunting\_of\_the\_Snark}).
It will usually be denoted by $q$ in these notes.
\end{definition}
So, at this point, we have a bag of \(q\) points denoted
\(E( {\mathbb{F}}_{p} )\). However, right now it only has the
structure of a set.

The beauty of elliptic curves is that it is possible to define an
\textbf{addition} operation on the curve; this is called the group law
on the elliptic curve.\footnote{https://en.wikipedia.org/wiki/Elliptic\_curve\#The\_group\_law}
This addition will make \(E( {\mathbb{F}}_{p} )\) into an
abelian group whose identity element is the point at infinity \(O\).
This addition can be formalized as a \emph{group law}.
...
computer to compute it. We provide details below for the interested
reader.
...
\begin{itemize}
\item
  Given a point \(P = ( x_{P},y_{P} )\), its negative is just
  \(- P = ( x_{P}, - y_{P} )\).
\item
  To add two points \(P\) and \(Q\), compute the line through the two
...
\(E( {\mathbb{F}}_{p} )\) with the additional structure of an
...
\begin{theorem}
The group BN254 is isomorphic to \({\mathbb{Z}}/q{\mathbb{Z}}\): Let
\(E\) be the BN254 curve. We have the isomorphism of abelian groups
\[E( {\mathbb{F}}_{p} ) \cong {\mathbb{Z}}/q{\mathbb{Z}}.\]
\end{theorem}

In these notes, this isomorphism will basically be a standing
assumption. Moving forward, we will abuse notation slightly and just
write \(E\) instead of \(E( {\mathbb{F}}_{p} )\). In fancy
language, \(E\) will be a one-dimensional vector space over
\({\mathbb{F}}_{q}\). In less fancy language, we will be working with
points on \(E\) as black boxes. We will be able to add them, subtract
them, and multiply them by arbitrary scalars from \({\mathbb{F}}_{q}\).

Consequently --- and this is important --- \textbf{one should think of
\({\mathbb{F}}_{q}\) as the base field for all our cryptographic
primitives} (despite the fact that the coordinates of our points are in
\({\mathbb{F}}_{p}\)).
...
\subsection{Discrete logarithm}\label{discretelog}

For our systems to be useful, rather than relying on factoring, we will
rely on the so-called \emph{discrete logarithm assumption}.

\begin{assumption}
\label{ddh}
Discrete logarithm assumption: Let \(E\) be the BN254 curve (or another
standardized curve). Given arbitrary nonzero \(g,g' \in E\), the
\emph{discrete logarithm problem} asks you to find an integer \(n\) such
that \(n \cdot g = g'\).

Experience suggests that the discrete logarithm problem is hard: In
general, we do not know a fast algorithm to solve it. The \emph{discrete
logarithm assumption} says that no such algorithm exists.
\end{assumption}
...
\(n \in {\mathbb{F}}_{q}\) interchangeably. In other words, \(n\) will
...
squaring.\footnote{https://en.wikipedia.org/wiki/Exponentiation\_by\_squaring}
...
\subsubsection{Curves other than BN254}
...
holds. In such a situation, \(E( {\mathbb{F}}_{p} )\) will
indeed be an abelian group once the identity element \(O = (0,\infty)\)
is added in.

How large is \(E( {\mathbb{F}}_{p} )\)? There is a theorem
called Hasse\textquotesingle s theorem\footnote{https://en.wikipedia.org/wiki/Hasse\textquotesingle s\_theorem\_on\_elliptic\_curves}
that states the number of points in \(E( {\mathbb{F}}_{p} )\)
is between \(p + 1 - 2\sqrt{p}\) and \(p + 1 + 2\sqrt{p}\). But there is
no promise that \(E( {\mathbb{F}}_{p} )\) will be
\emph{prime}; consequently, it may not be a cyclic group either. So
among many other considerations, the choice of constants in BN254 is
engineered to get a prime order.

There are other curves used in practice for which
\(E( {\mathbb{F}}_{p} )\) is not a prime, but rather a small
multiple of a prime. The popular Curve25519\footnote{https://en.wikipedia.org/wiki/Curve25519}
is such a curve that is also believed to satisfy
Assumption \ref{ddh}. Curve25519 is defined as
\[Y^{2} = X^{3} + 486662X^{2} + X\] over \({\mathbb{F}}_{p}\) for the
prime \(p := 2^{255} - 19\). Its order is \(8\) times a large prime
\[q' := 2^{252} + 27742317777372353535851937790883648493.\] In that case,
to generate a random point on Curve25519 with order \(q'\), one will
usually take a random point on the curve and multiply it by \(8\).
...
\subsubsection{Example application: EdDSA signature scheme}\label{eddsa}

We will show how Assumption \ref{ddh} can be used to construct a
signature scheme that replaces RSA. This scheme is called
EdDSA,\footnote{https://en.wikipedia.org/wiki/EdDSA} and it is used
quite frequently (e.g., in OpenSSH and GnuPG). One advantage it has over
RSA is that its key size is much smaller: both the public and private
key are 256 bits. (In contrast, RSA needs 2048-4096 bit keys for
comparable security.)
...
\(\lbrack n\rbrack\), we cannot get \(n\). You can almost think of the
notation as an ``armor'' on the integer \(n\): It conceals the integer,
but still allows us to perform (armored) addition:
\[\lbrack a + b\rbrack = \lbrack a\rbrack + \lbrack b\rbrack.\] In other
words, \(n \mapsto \lbrack n\rbrack\) viewed as a map
\({\mathbb{F}}_{q} \rightarrow E\) is \({\mathbb{F}}_{q}\)-linear.

So now suppose Alice wants to set up a signature scheme.

\begin{algorithm}[EdDSA public and secret key]
\leavevmode
\begin{enumerate}
\item
  Alice picks a random integer \(d \in {\mathbb{F}}_{q}\) as her
  \emph{secret key} (a piece of information that she needs to keep
  private for the security of the protocol).
\item
  Alice publishes \(\lbrack d\rbrack \in E\) as her \emph{public key} (a
...
\(\mathsf{\text{msg}}\), given her published public key
\(\lbrack d\rbrack\).

\begin{algorithm}[EdDSA signature generation]
\leavevmode
...
\begin{enumerate}
\item
  Alice picks a random scalar \(r \in {\mathbb{F}}_{q}\) (keeping this
  secret) and publishes \(\lbrack r\rbrack \in E\).
\item
  Alice generates a number \(n \in {\mathbb{F}}_{q}\) by hashing
  \(\mathsf{\text{msg}}\) with all public information, say
  \[n := \operatorname{hash}(\lbrack r\rbrack,\mathsf{\text{msg}},\lbrack d\rbrack).\]
\item
  Alice publishes the integer \[s := (r + dn)\operatorname{mod}q.\]
\end{enumerate}
...
\begin{algorithm}[EdDSA signature verification]
\leavevmode
For Bob to verify a signature
\(( \lbrack r\rbrack,s )\) for \(\mathsf{\text{msg}}\):

\begin{enumerate}
\item
  Bob recomputes \(n\) (by also performing the hash) and computes
  \(\lbrack s\rbrack \in E\).
\item
  Bob verifies that
  \(\lbrack r\rbrack + n \cdot \lbrack d\rbrack = \lbrack s\rbrack\).
\end{enumerate}
\end{algorithm}

An adversary cannot forge the signature even if they know \(r\) and
\(n\). Indeed, such an adversary can compute what the point
\(\lbrack s\rbrack = \lbrack r\rbrack + n\lbrack d\rbrack\) should be,
...
pick \(r = \operatorname{hash}(d,\mathsf{\text{msg}})\); this has the
...
\subsubsection{Example application: Pedersen commitments}
\label{pedersen}

A \emph{commitment scheme} is a protocol where Alice wants to commit
some value \(x\) to Bob that is later revealed. Typically Alice gives
Bob some ``commitment'' \(\operatorname{Com}(x)\) and later reveals
...
\(\operatorname{Com}(x)\)). The KZG scheme we are building towards will
...
A multivariable generalization of Assumption \ref{ddh} is that if
...
\(( a_{1},\ldots,a_{n} ) \neq ( b_{1},\ldots,b_{n} ) \in {\mathbb{F}}_{q}^{n}\)
such that
...
we will say they are a \emph{computational basis over $\FF_q$}.
\end{definition}
...
is called the \emph{Pedersen commitment} to our vector $\vec{a}$.
\end{definition}
...
entire vector \(\overset{\rightarrow}{a}\) one can compute the Pedersen
commitment easily.
