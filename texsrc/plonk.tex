...
Q_1\left( x_{1},\ldots,x_{k} \right) & = 0 \\
 \vdots \\
Q_{m}\left( x_{1},\ldots,x_{k} \right) & = 0.
...
y_{1} & = \left( 1 - x_{42} \right) \cdot x_{17}, & \quad & 0 = y_{1} \cdot x_{53} \\
y_{2} & = \left( 1 - x_{19} \right) \cdot \left( 1 - x_{52} \right) & \quad & 0 = y_{2} \cdot \left( 1 - x_{75} \right) \\
...
\[\left( q_{L,i},q_{R,i},q_{O,i},q_{M,i},q_{C,i} \right) = (1,1, - 1,0,0),\]
we get an ``addition'' gate \(a_{i} + b_{i} = c_{i},\) while if we set
\[\left( q_{L,i},q_{R,i},q_{O,i},q_{M,i},q_{C,i} \right) = (0,0, - 1,1,0),\]
we get a ``multiplication'' gate \(a_{i}b_{i} = c_{i}.\) Finally, if
\(\kappa\) is any constant, then
\[\left( q_{L,i},q_{R,i},q_{O,i},q_{M,i},q_{C,i} \right) = (1,0,0,0, - \kappa),\]
...
\[\begin{aligned}
x \cdot x & = \left( x^{2} \right)\text{ (multiplication)} \\
t & = 2\text{ (constant)} \\
\left( x^{2} \right) + t & = y\text{ (addition)}.
\end{aligned}\]

We will assign the variables \(a_{i},b_{i},c_{i}\) for these two gates
by looking at the equations: \[\begin{aligned}
\left( a_{1},b_{1},c_{1} \right) & = \left( x,x,x^{2} \right) \\
\left( a_{2},b_{2},c_{2} \right) & = (t = 2,0,0) \\
\left( a_{3},b_{3},c_{3} \right) & = \left( x^{2},t = 2,y \right).
\end{aligned}\]
...
\[Z(X) = (X - \omega)\cdots\left( X - \omega^{n} \right) = X^{n} - 1,\]
...
\begin{algorithm}[Commitment step of PLONK]
\leavevmode
\begin{enumerate}
\item
