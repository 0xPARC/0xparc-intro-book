...
\begin{problem}
Suppose we have polynomials \(P,Q \in {\mathbb{F}}_{q}[ X]\)
...
Equation \ref{permcheck-poly}: If the two sides of
Equation \ref{permcheck-poly} are not the same
...
Equation \ref{permcheck-poly} are extremely small.

We can then get a proof of
Equation \ref{permcheck-poly} using the technique of
...
\(F_{P} \in {\mathbb{F}}_{q}[ T]\) such that
\[\begin{aligned}
...
\(F_{Q} \in {\mathbb{F}}_{q}[ T]\) is defined analogously.

So to prove Equation \ref{permcheck-poly}, the
...
\item
  Peggy uses Algorithm \ref{root-check} to prove all of the following statements:
...
\(\mu \in {\mathbb{F}}_{q}\), we also have 
\begin{equation}
\label{copy2}
\begin{aligned}
 & \begin{pmatrix}
...
\item
  ...
  \(\sigma_{a},\sigma_{b},\sigma_{c} \in {\mathbb{F}}_{q}[ X]\)
...
\item
  Peggy uses Algorithm \ref{root-check} to prove
