...
``How can we provably hide access patterns while preserving
efficiency?''
...
definitions}\label{oblivious-ram-problem-definitions}
...
\(\left( \mathtt{\text{read}},\mathtt{\text{addr}} \right)\) or
\(\left( \mathtt{\text{write}},\mathtt{\text{addr}},\mathtt{\text{data}} \right)\).
...
\(\Theta\left( N\log N \right)\) bits to store --- but later we can
...
\begin{figure}
\centering
{\includegraphics[keepaspectratio]{binaryoram16-evict.png}}
...
We present the algorithm's pseudo-code in Algorithms~{[}alg{]}:access
and {[}alg{]}:evict.

\phantomsection\label{alg:access}
strong(Algorithm):\\
The procedure
\(\mathtt{\text{Access}}(\mathtt{\text{op}},\mathtt{\text{addr}},\mathtt{\text{data}}^{*})\)
where \(\mathtt{\text{op}} = \mathtt{\text{read}}\) or
\(\mathtt{\text{op}} = \mathtt{\text{write}}\)

\textbf{Assume:} Each block is of the form
\(\left( \mathtt{\text{addr}},\mathtt{\text{data}},l \right)\) where
\(l\) denotes the block's current designated path.

\begin{enumerate}
\item
  Let \(l^{*}\) be a random value from \(1\) to \(N\). Assign
  \(l \leftarrow \mathtt{\text{position}}\left\lbrack \mathtt{\text{addr}} \right\rbrack\),
  \(\mathtt{\text{position}}\left\lbrack \mathtt{\text{addr}} \right\rbrack \leftarrow l^{*}\).
\item
  ...
  \setcounter{enumii}{2}
  \item
    Scan \(\mathtt{\text{bucket}}\), and if
    \(\left( \mathtt{\text{addr}},\mathtt{\text{data}}_{0},\_ \right) \in \mathtt{\text{bucket}}\)
    then let
    \(\mathtt{\text{data}}^{*} \leftarrow \mathtt{\text{data}}_{0}\) and
    remove this block from \(\mathtt{\text{bucket}}\).
  \end{enumerate}
\item
  End for
\item
  If \(\mathtt{\text{op}} = \mathtt{\text{read}}\) then add
  \(\left( \mathtt{\text{addr}},\mathtt{\text{data}}^{*},l^{*} \right)\)
  to the root bucket; else add
  \(\left( \mathtt{\text{addr}},\mathtt{\text{data}},l^{*} \right)\) to
  the root bucket.
\item
  Call the \(\mathtt{\text{Evict}}\) subroutine.
\item
  Return \(\mathtt{\text{data}}^{*}\).
\end{enumerate}

\phantomsection\label{alg:evict}
strong(Algorithm):\\
The procedure \(\mathtt{\text{evict}}\)
...
\item
  ...
  \setcounter{enumii}{1}
  \item
    \(\mathtt{\text{bucket}}_{0}\),
    \(\mathtt{\text{bucket}}_{1} \leftarrow\) randomly choose \(2\)
    distinct buckets in the level \(d\) (for the root level, pick one
    bucket).
  \item
    For
    \(\mathtt{\text{bucket}} \in \{\mathtt{\text{bucket}}_{0},\mathtt{\text{bucket}}_{1}\}\)
    do:
    ...
    \item
      For each of the two children of \(\mathtt{\text{bucket}}\) in a
...
\item
  ...
  more than \(\left( \log N \right)^{2}\) blocks are assigned to any one
...
\[\Pr\left\lbrack \mathrm{\text{number of items in queue}} > R \right\rbrack \leq \exp\left( \Omega( - R) \right).\]
...
As long as the block size is at least \(\Omega\left( \log N \right)\)
bits, every time we recurse, the ORAM's size reduces by a constant
factor; and thus \(O\left( \log N \right)\) levels of recursion would
suffice.
...
that achieves \(O\left( \alpha\log^{3}N \right)\) cost for each access:
each logical request will translate to
\(O\left( \alpha\log^{3}N \right)\) physical accesses; and moreover, the
client is required to store only \(O(1)\) number of blocks.

Note that in the total cost \(O\left( \alpha\log^{3}N \right)\), an
...
cost of each access is \(O\left( \alpha\log^{2}N \right)\) for an
