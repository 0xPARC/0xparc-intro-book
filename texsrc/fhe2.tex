...
In {[}lwe-small{]} we saw how even a small case of this problem
...
set'' in {[}lwe-small{]}. In fact, that ``blue set'' will be exactly the
public key.
...
in a moment why we insist that \(\mu\) is one of these two values.) The
...
\(\mu\) is either \(0\) or \(5\) (and we are working modulo \(q = 11\)),
that is just enough to determine \(\mu\) uniquely.
...
\(r\) is bigger than the error bound (right now that is \(m\)), which
