...
the verification equation is that
\begin{align*}
  &\pair( [Q(s)], [ (s - 1)(s - 2)\ldots(s - 9) ] ) \\
  &\quad = \pair( [P(s)] - 100[s^2], [1] ).
\end{align*}
...
\item
  ...
  \[P(X) - f(X) = ( X - z_{1} )\cdots( X - z_{n} ) \cdot Q(X).\]
\item
...
\item
  Victor verifies
\begin{align*}
    &\pair([Q(s)], [ ( s - z_1 )( s - z_2 )\ldots( s - z_n ) ]) \\
    &\quad = \pair([P(s)] - [f(s)], [1]).
\end{align*}
\end{enumerate}
\end{algo}
