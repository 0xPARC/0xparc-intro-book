...
Remember the notation \([ n] := n \cdot g \in E\) defined in
...
Peggy has a polynomial \(P(X) \in \mathbb{F}_{p}[X]\). To
commit to it:
...
\item
  ...
  \(Q(X) \in {\mathbb{F}}_{q}[ X]\) such that
  \[P(X) - y = (X - z)Q(X).\]
\item
...
\begin{equation}
\label{kzg-verify}  
\operatorname{pair}([ Q(s) ],[ s] - [ z]) = \operatorname{pair}([ P(s) ] - [ y],[ 1])
\end{equation}
...
\item
  Victor verifies 
\[  \operatorname{pair}([ Q(s) ],[ ( s - z_{1} )( s - z_{2} )\ldots( s - z_{n} ) ]) = \operatorname{pair}([ P(s) ] - [ f(s) ],[ 1]). \]
\end{enumerate}
\end{algo}
...
\item
  ...
  \[Z(X) := \prod_{z \in S}(X - z) \in {\mathbb{F}}_{q}[ X].\]
\item
