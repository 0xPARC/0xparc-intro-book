...
but we 'll comment briefly in Section \ref{pairing-friendly} on what curves it
can be done on.
...
We can verify more complicated claims as well by putting things together.
For example, suppose Peggy wants to convince Victor that $y = x^{3} + 2$, where
Peggy has sent Victor elliptic curve points $[x]$ and $[y]$.

\begin{example}{}{Verifying $y = x^3+2$ from $[x]$ and $[y]$}
Peggy additionally sends to Victor $[x^2]$ and $[x^3]$.

Then given $[x]$, $[x^2]$, $[x^3]$, and $[y]$, Victor verifies that:
\begin{itemize}
\item $\pair([x^{2}],[1]) = \pair([x],[x])$
\item $\pair([x^{3}],[1]) = \pair([x^2],[x])$
\item $[y] = [x^3] + 2[1]$.
\end{itemize}
\end{example}
...
This section is an aside explaining how the so-called
``embedding degree'' affects pairing, and can be skipped safely.

If we chose $E$ to be BN254, the following property holds:
\begin{proposition}{BN254 has embedding degree $12$}{}
For $(p,q)$ as in BN254, the smallest integer $k$ such that $q$
divides $p^{k} - 1$ is $k = 12$.
\end{proposition}
This integer $k$ is called the \alert{embedding degree}.
...
of the pairing is based on the \emph{Weil pairing}\footnote{\url{https://en.wikipedia.org/wiki/Weil\_pairing}.\\
The idea to use the Weil pairing in cryptography goes back to Dan Boneh and Matthew Franklin: \\
