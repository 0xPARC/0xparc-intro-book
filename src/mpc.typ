...
their respective inputs. Furthermore, they would like to keep their secret
...
$f (a , b)$, but Alice should not learn anything about $b$ (other than
...
- $f$ is a binary circuit, which takes in $m + n$ bits, and runs them
  ...
  public outputs of the circuit. Without loss of generality, let us also
...
- An _oblivious transfer_ is a protocol where Alice has two messages,
  ...
  Bob does not learn the passwords for any other inputs,
  and Alice does not find out which password she sent to Bob.

In slightly more detail:

1. Whatever the function $f$ is, we will assume that it takes $m+n$ bits of input
   $a_1, dots, a_m$ and $b_1, dots, b_n$,
   and that it is computed by some sort of circuit
...
3. Now, Alice is going to "garble" the circuit $f_a$.
   Once it is garbled, Bob will not be able to see how it works.
   He will only be able to plug his own input $(b_1, dots, b_n)$
   into the circuit.
4. To prevent Bob from plugging other inputs in as well,
   ...
   but he will not learn anything else about how the circuit works.
5. Now, Alice has all the passwords for all the possible inputs, but how can she give Bob the password for $(b_1, dots, b_n)$?
   Alice does not want to let Bob have any other passwords --
   and Bob is not willing to tell Alice which password he is asking for.
...
What does that mean? Let us say the gate has two input bits,
...
But if you do not know the password $P_(x, y)$,
assuming $Enc$ is a suitably secure encryption scheme,
you will not learn anything about the value $G_(x, y)$
from its encryption.

== Chaining garbled gates

The next step is to combine a bunch of garbled gates into a circuit.
We will need to make two changes to the protocol.
...
2. To keep the functionality of the circuit hidden,
  we do not want Bob to learn anything about the structure of
  ...
  we will have the gate just output $P_z$.
  ...
  not by the bits $(x, y)$.
  But we do not want Bob to learn the other passwords from the table!
  ...
  For simplicity, we will assume it is an AND gate --
...
1. Suppose Bob's input bits are 0 (on the left) and 1 (on the right).
  Bob does not know he has 0 and 1 (but we do!).
...
he knows the password for his bit, but he does not know his bit.
...
Bob needs to learn the passwords for all of his input bits. Let us frame the problem for just a single bit.
...
This is where _oblivious transfer_ comes in, which we will see in @ot.
