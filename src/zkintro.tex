...
\item Maybe it is just a really long, annoying calculation, and Victor does
...
The name stands for:
\begin{description}
\item[Succinct] The proof length is short (in fact, it is a constant
  length, independent of how long the problem is).
\item[Non-interactive.] The protocol does not require back-and-forth
  communication.
\item[Argument.] Basically a proof. There is a technical difference,
  but we will not worry about it.
\item[of Knowledge.] The proof does not just show the system of
...
$\sha(M) = \ 0xa91af3ac...$, but you do not want to
