...
\begin{itemize}
\item Maybe Peggy wants to keep part of the calculation secret. Maybe her
...
\item Maybe it's just a really long, annoying calculation, and Victor does
  not have the energy to check it all line-by-line.
\end{itemize}

A \alert{SNARK}\footnote{The first SNARK was introduced by Silvio Micali in 2000.
We will explain a more recent scheme, PLONK, from 2019. \\
PLONK: \url{https://eprint.iacr.org/2019/953.pdf}}
 lets Peggy (the ``prover'') send Victor (the ``verifier'')
a short proof that she has indeed done the calculation correctly.
The proof will be much shorter than the original calculation,
and Victor's verification is much faster.
(As a tradeoff, writing a
SNARK proof of a calculation is much slower than just doing the
calculation.)

The name stands for:
\begin{description}
\item[Succinct] The proof length is short (in fact, it's a constant
  length, independent of how long the problem is).
\item[Non-interactive.] The protocol does not require back-and-forth
  communication.
\item[Argument.] Basically a proof. There's a technical difference,
  but we won't worry about it.
\item[of Knowledge.] The proof doesn't just show the system of
  equations has a solution; it also shows that the prover knows one.
\end{description}

We won't discuss it here, but it is also possible and frequently
useful to make a \emph{zero knowledge (zk) SNARK}, or \alert{zkSNARK}.
This gives Peggy a guarantee that Victor will not
...
equations. Sounds pretty boring, unless you're an algebra student.
...
$\sha(M) = \ 0xa91af3ac...$, but you don't want to
...
One recent application along these lines is TLSNotary.\footurl{https://tlsnotary.org}
