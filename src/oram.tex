...
physical access is a read or write. We don't say anything about the
...
In light of Algorithm \ref{naive2}, we will require that the client store only a small amount of data (constant or polylogarithmic in $N$).
...
We will give a heuristic argument that doesn't require any specialized
knowledge.
...
we expect that it's very unlikely for a lot of blocks to accumulate in
any one bucket.
...
Unlike the above binary-tree ORAM, in Path ORAM, every bucket is
constant in size (maybe 4 or 5), except the root bucket which is
...
RAM-model MPC, and the idea is that we first translate the RAM into an
