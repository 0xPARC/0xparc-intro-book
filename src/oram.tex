...
This is the problem Oblivious RAM sets out to solve.\footnote{ORAM was first proposed by Goldreich and Ostrowsky, in papers in 1987 and 1996.}
...
Signal has a secure processor (like Intel SGX) on its server; you can think of this as a secure hardware sandbox (called an \emph{enclave})
...
\begin{algo}{Trivial solution 1: Full scan.}{}
...
\begin{algo}{Trivial solution 2: Store everything on the client.}{}
...
In light of ``Trivial solution 2,'' we will require that the client store only a small amount of data (constant or polylogarithmic in $N$).
...
\begin{algo}{$\texttt{The procedure Access}(\texttt{op}, \texttt{addr}, \texttt{data}^*)$ where $\texttt{op} = \texttt{read}$ or $\texttt{op} = \texttt{write}$}{access}
...
in particular Burke's theorem.\footurl{https://en.wikipedia.org/wiki/Burke\%27s\_theorem}
...
introduce an improved version called Path 
