...
To be concrete, let's consider two examples of what protocols designed
...
To draw an analogy, it's like going from single-purpose hardware (like
...
\emph{proof}. Specifically, Alice's signature is a proof that ``I
...
In the spirit of programmable cryptography, a \alert{SNARK} generalizes
this concept as a ``proof system'' protocol that can produce an efficient
proof of an \emph{arbitrary} statement of the form:
\begin{quote}
...
we can also imagine that you care about security a lot and you really don't
want the translating service to know anything about your text at all.
...
database is so large that you can't store it yourself -- and you don't
trust the server it's stored on.

First off, you'll encrypt the data, so the server can't read it. But
...
This book was made possible with the help of members of the 0xPARC community.

We would like to thank Brandon Wang, Janabel Xia, and Claire Zhang for
contributions to an early set of notes that evolved into this book; Alex
Allain for many comments and corrections; and Caroline Ballegaard, Sebastian
Clark, Holden Mui, Arturo Rojas, and Mihir Singhal for assistance with
typesetting, printing, and producing this edition.
