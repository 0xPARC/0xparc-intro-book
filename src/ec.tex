% Don't delete the blank line at the top of this file.
...
will talk about the KZG polynomial commitment scheme, which plays an important
...
straight to SNARKs in the next chapter.
...
The BN254 specification fixes a specific\footnote{If you must know, the
  ...
  & \hspace{20pt} 96311157297823662689037894645226208583 \\
  q & \defeq 36x^{4} + 36x^{3} + 18x^{2} + 6x + 1 \\
  & = 218882428718392752222464057452572750885 \\
  & \hspace{20pt} 48364400416034343698204186575808495617.
  \end{align*}
}
...
who proposed a family of such curves in 2006.\footurl{https://eprint.iacr.org/2005/133.pdf}
...
Okay, actually, that's a white lie: Conventionally, there is one additional
...
The beauty of elliptic curves is that it's possible to define an
...
This group law involves some heavy algebra. It's not important to
...
Moving forward, we'll abuse notation slightly and just
...
We'll be able to add them, subtract them, and multiply them by arbitrary scalars from $\FF_{q}$.
...
We'll show how Assumption \ref{tcb:ddh} can be used to construct a
...
$s$. It's therefore imperative that $r$ isn't known to Bob nor
...
curves to commit \textbf{numbers} with something called a \emph{Pedersen
...
$E$ with order $q$, then it's computationally infeasible to find
...
we'll say they're a \alert{computational basis over $\FF_q$}.
\end{definition}
...
ends up being injective \emph{in practice} (because we can't find collisions).
...
``linearly independent'' now being phrased as ``we can't find a
...
The Pedersen commitment\footnote{Introduced by Torben Pryds Pedersen in 1992.} is thus a sort of hash function: given the group
...
We won't use Pedersen commitments in this book, but they will be
...
So, let's get started. The equation of $E$ is cubic -- the
...
geometric operations we're describing -- like finding the intersection
of a curve with a line -- can be translated into algebra, and then you
just do the algebra in your finite field. We'll come back to this.)
...
so it has (at most) three roots. In fact if it has two roots, it's
guaranteed to have a third (because you can factor out the first two
roots, and then you're left with a linear factor).
...
looks like there are only two intersection points. After all, we're
...
this lies in projective geometry, but we won't get into it.) So the
...
example), but we won't get into it.
