...
We will assume the messages are essentially unrelated to each other
...
We will see two simple protocols to achieve this.

(In fact, for two-party computation,
we only need "1-of-2 OT":
Alice has $x_1$ and $x_2$, and she wants to send one of
those two to Bob.
But "1-of-$n$ OT" is not any harder, so we will do 1-of-$n$.)

== Commutative encryption

Let us imagine that Alice and Bob
...
it does not matter which order Alice and Bob decrypt the message in --
they will still get the original message back.

A metaphor for commutative encryption
is a box that is locked with two padlocks.
...
What is special about commutative encryption
is that Bob's lock does not block Alice from unlocking her own --
...
At this point, Bob cannot read any of the messages,
because he does not know the keys.
No problem!
...
Alice does not know Bob's key $b$,
so she cannot learn anything about the message he encrypted --
...
In total, Alice sent two messages to Bob (Step 1 and 3),
and Bob sent one message to Alice (Step 2).

We can do better, using public-key cryptography.

Let us start with a simplified protocol that is not quite secure.
...
as long as she cannot distinguish a true public key
...
like we just said, Alice cannot tell random garbage from a true public key!
And then Bob would be able to decrypt all $n$ messages $x_1, dots, x_n$.

But there is a simple trick to fix it.
...
It is a standard assumption in public-key cryptography that Bob cannot do this:
There is no algorithm that reads in a public key and spits out the corresponding private key.
...
and it is very hard (we assume) for Bob to find a private key for a random public key.
