...
she does not want to tell him $x$. What Alice wants is a _fully homomorphic encryption (FHE)_ protocol, meaning:
...
_Leveled FHE_ is a weaker version of FHE. Like FHE, leveled FHE
...
will not give the right answer anymore. But as long as you are careful not to go over the error limit, you can
add ciphertexts with confidence.

For our leveled FHE protocol, our message will be a bit (either 0 or 1) and
our operations will be the logic gates AND and NOT. Because any logic circuit can be built out of AND and NOT gates, we will be able to perform arbitrary calculations
...
Anyway, we will start with the learning-with-errors problem
(@lwe) and how to build cryptography on top of it (@lwe-crypto)
before we get back to leveled FHE.
