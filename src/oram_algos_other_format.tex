...
\item
  ...
  $l \leftarrow\texttt{position}\left[ \texttt{addr} \right]$,
  $\texttt{position}\left[ \texttt{addr} \right] \leftarrow l^{*}$.
\item
...
\item
  ...
  \item
    ...
    \item
      $\textsf{block} \defeq$ pop a real block from
      $\textsf{bucket}$ if one exists; else
      $\textsf{block} \defeq (\bot,\bot,\bot)$.
    \item
      For each of the two children of $\texttt{bucket}$ in a
      fixed order: scan the child bucket reading and writing every
      block. If $\textsf{block}$ is real and wants to go to the
      child, write $\textsf{block}$ to an empty slot in the
      child bucket.
    \end{enumerate}
  \item
    End for
