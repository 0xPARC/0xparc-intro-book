...
inputs. Furthermore, they'd like to keep their secret values hidden
...
shouldn't learn anything about $b$ (other than what could be learned
by knowing both $a$ and $f(a,b)$), and likewise for Bob.

Here is our problem setting, slightly more formally:\footnote{The idea of multi-party computation
(two-party computation is a special case)
goes back to work of Goldreich, Micali, and Wigderson: \\
 \url{https://www.math.ias.edu/~avi/PUBLICATIONS/MYPAPERS/GMW87/GMW87.pdf}}
\begin{goal}{}{}
\begin{itemize}
...
\item $f$ is a binary circuit, which takes in $m + n$ bits, and runs
  ...
  the public outputs of the circuit. Without loss of generality, let's
...
Yao's garbled circuits\footnote{Introduced by Andrew C.\ Yao in 1982: \\ \url{https://research.cs.wisc.edu/areas/sec/yao1982-ocr.pdf}}
leads to one of the most well-known 2PC protocols.
...
\item \alert{Oblivious transfer}\footnote{Introduced by Michael Rabin in 1981: \url{https://eprint.iacr.org/2005/187.pdf}}
is a protocol where Alice has two
  ...
  doesn't learn the passwords for any other inputs, and Alice doesn't
...
\begin{enumerate}
\item
  Whatever the function $f$ is, we'll assume that it takes $m + n$
  bits of input $a_{1},\ldots,a_{m}$ and $b_{1},\ldots,b_{n}$, and
  that it's computed by some sort of circuit made of AND, OR and NOT
  gates.
\item
  ...
  $b_{1},\ldots,b_{n}$.
\item
  Now, Alice is going to ``garble'' the circuit $f_{a}$. Once it's
  garbled, Bob won't be able to see how it works. He'll only be
...
\item
  ...
  $f_a( b_{1},\ldots,b_{n}) = f(a,b)$, but he won't
...
\item
  ...
  $( b_{1},\ldots,b_{n})$? Alice doesn't want to let Bob
  have any other passwords -- and Bob isn't willing to tell Alice which
  password he is asking for. This is where we will use ``oblivious
  transfer.''
\end{enumerate}

Now, we will flesh out this outline, starting with garbled circuits.
...
What does that mean? Let's say the gate has two input bits, so there
...
But if you don't know the password $P_{x,y}$, assuming
$\Enc$ is a suitably secure encryption scheme, you won't
learn anything about the value $G_{x,y}$ from its encryption.

\subsection{Chaining garbled gates}

The next step is to combine a bunch of garbled gates into a circuit. We'll need to make some small changes to the protocol.
...
\item
  To keep the functionality of the circuit hidden, we don't want Bob to
  ...
  $z$ and the password $P_{z}$, we'll have the gate just output
  $P_{z}$.
  ...
  the bits $(x,y)$. But we don't want Bob to learn the other
  ...
  \item
    ...
    input $(x,y)$.
  \item
    If Bob doesn't know the passwords, he can't learn them by looking
    at the table.
  \end{itemize}

  Of course, the solution is to use a hash function! So here is the new
  version of our garbled gate. For simplicity, we'll assume it's an
...
\item
  ...
  doesn't know he has 0 and 1 (but we do!). Bob knows his left password
...
\item
  ...
  \[\hash(P_{0}^{\text{left}},P_{1}^{\text{right}}).\]
\item
  Then, Bob finds the row of the table indexed by
...
but he doesn't know his bit. So we can chain together as many of these
...
learn the passwords for all of his input bits. Let's frame the problem
...
This is where \emph{oblivious transfer} comes in, which we'll see in
Section \ref{ot}.
