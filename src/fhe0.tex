...
What Alice wants is a \alert{fully homomorphic encryption (FHE)}\footnote{Fully
homomorphic encryption was introduced by Craig Gentry;
the protocol here was introduced by Gentry, Sahai, and Waters in 2013: \url{https://eprint.iacr.org/2013/340.pdf}}
protocol,
meaning she wants a protocol that can support the following:

\begin{goal}{}{}
FHE should make the following possible:
\begin{enumerate}
\item Alice encrypts $x$ and sends Bob $\Enc(x)$.
\item Bob then ``applies $f$ to the ciphertext'' and obtains
  $\Enc\left( f(x) \right)$, sending it to Alice.
\item Alice decrypts $\Enc\left( f(x) \right)$ to learn $f(x)$.
\end{enumerate}
\end{goal}

\alert{Leveled FHE} is a weaker version of FHE. Like FHE, leveled FHE
...
As a silly example, imagine your message is a whole number
...
number, no secret key required. Yep, you're right. The full scheme
(Section \ref{lwe-crypto}) is more complicated. But it still has this
``rounding-off-errors'' feature, and that's what we want to focus on
right now.)
...
rounding procedure won't give the right answer anymore. But as long
as you're careful not to go over the error limit, you can add
...
logic circuit can be built out of AND and NOT gates, we'll be able to
perform arbitrary calculations within the FHE encryption.

Our protocol uses a cryptosystem built from a problem called
``\alert{learning with errors}.''
``Learning with errors'' is kind of a strange name; it
would make more sense to call it ``approximate linear algebra modulo $q$''.
