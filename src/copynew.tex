\section{Copy constraints in PLONK}\label{copy-constraints}
...
strong(Problem):\\
Suppose we have polynomials \(P,Q \in {\mathbb{F}}_{q}\lbrack X\rbrack\)
which encode two vectors of values \[\begin{aligned}
\overset{\rightarrow}{p} & = \langle P\left( \omega^{1} \right),P\left( \omega^{2} \right),\ldots,P\left( \omega^{n} \right)\rangle \\
\overset{\rightarrow}{q} & = \langle Q\left( \omega^{1} \right),Q\left( \omega^{2} \right),\ldots,Q\left( \omega^{n} \right)\rangle.
...
\[{⬭(a_{1})} = {⬭(a_{4})} = {⬭(c_{4})}\quad\text{and }\quad{▭(b_{2})} = {▭(c_{1})}.\]
...
\end{pmatrix} \\
 = & \begin{pmatrix}
{⬭(a_{4})} + \omega^{1}\mu & b_{1} + \eta\omega^{1}\mu & {▭(b_{2})} + \eta^{2}\omega^{1}\mu \\
a_{2} + \omega^{2}\mu & {▭(c_{1})} + \eta\omega^{2}\mu & c_{2} + \eta^{2}\omega^{2}\mu \\
a_{3} + \omega^{3}\mu & b_{3} + \eta\omega^{3}\mu & c_{3} + \eta^{2}\omega^{3}\mu \\
{⬭(c_{4})} + \omega^{4}\mu & b_{4} + \eta\omega^{4}\mu & {⬭(a_{1})} + \eta^{2}\omega^{4}\mu
\end{pmatrix}.
...
\begin{pmatrix}
a_{1} + {⬭({\eta^{2}\omega^{4}\mu})} & b_{1} + \eta\omega^{1}\mu & c_{1} + {▭({\eta\omega^{2}\mu})} \\
a_{2} + \omega^{2}\mu & b_{2} + {▭({\eta^{2}\omega^{1}\mu})} & c_{2} + \eta^{2}\omega^{2}\mu \\
a_{3} + \omega^{3}\mu & b_{3} + \eta\omega^{3}\mu & c_{3} + \eta^{2}\omega^{3}\mu \\
a_{4} + {⬭({\omega^{1}\mu})} & b_{4} + \eta\omega^{4}\mu & c_{4} + {⬭({\omega^{4}\mu})}
\end{pmatrix}.
...
\sigma_{a}\left( \omega^{1} \right) = {⬭({\eta^{2}\omega^{4}})} & \sigma_{b}\left( \omega^{1} \right) = \eta\omega^{1} & \sigma_{c}\left( \omega^{1} \right) = {▭({\eta\omega^{2}})} \\
\sigma_{a}\left( \omega^{2} \right) = \omega^{2} & \sigma_{b}\left( \omega^{2} \right) = {▭({\eta^{2}\omega^{1}})} & \sigma_{c}\left( \omega^{2} \right) = \eta^{2}\omega^{2} \\
\sigma_{a}\left( \omega^{3} \right) = \omega^{3} & \sigma_{b}\left( \omega^{3} \right) = \eta\omega^{3} & \sigma_{c}\left( \omega^{3} \right) = \eta^{2}\omega^{3} \\
\sigma_{a}\left( \omega^{4} \right) = {⬭(\omega^{1})} & \sigma_{b}\left( \omega^{4} \right) = \eta\omega^{4} & \sigma_{c}\left( \omega^{4} \right) = {⬭(\omega^{4})}.
