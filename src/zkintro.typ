...
- Maybe it is just a really long, annoying calculation,
...
- _Succinct_: the proof length is short (in fact, it is a constant length,
...
- _Argument_: basically a proof.
  There is a technical difference, but we will not worry about it.
- _of Knowledge_: the proof does not just show the system of equations has a solution;
  it also shows that the prover knows one.

We will not discuss it here, but it is also possible and frequently useful to make a _zero knowledge (zk)_ SNARK. These are typically called "zkSNARKs." This gives Peggy a guarantee
...
Sounds pretty boring, unless you are an algebra student.
...
$op("sha")(M) = "0xa91af3ac..."$, but you do not want to reveal $M$.
