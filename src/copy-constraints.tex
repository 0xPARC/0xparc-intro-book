...
\overrightarrow{p} & = \langle P( \omega^{1} ),P( \omega^{2} ),\ldots,P( \omega^{n} )\rangle \\
\overrightarrow{q} & = \langle Q( \omega^{1} ),Q( \omega^{2} ),\ldots,Q( \omega^{n} )\rangle.
\end{aligned}\] Is there a way that one can quickly verify
$\overrightarrow{p}$ and $\overrightarrow{q}$ are the
...
Equation \eqref{permcheck-poly} aren't the same
...
Suppose Peggy has committed $\Com(P)$
and $\Com(Q)$.
...
\item
  ...
  Define $F_{Q}$ similarly. Peggy sends $\Com(F_{P})$
  and $\Com(F_{Q})$.
\item
...
Moving on to copy check, let's look at a concrete example where
$n = 4$. Suppose that our copy constraints were
\[ \redoval{(a_{1})} =
\redoval{(a_{4})} =
\redoval{(c_{4})}
\quad\text{and }\quad \bluerect{(b_{2})} = \bluerect{(c_{1})}.\]
(We've colored the variables that will move around for
...
Again, our goal is to make this into a \emph{single} equation. There's
...
such that $\eta^{2}$ doesn't happen to be a power of $\omega$, so
...
values $a_{i}$, $b_{i}$, and $c_{i}$ that don't satisfy all the
...
polynomials, after reintroducing the random challenge $\lambda$ from
permutation check. We're going to need six in all, three for each side
