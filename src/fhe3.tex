\subsection{Leveled FHE from LWE}\label{fhe}

\subsubsection{The main idea: approximate eigenvalues}
...
Recall the setup from Section \ref{lwe-crypto}: We are going pick some large
...
\[\mathbf{v} = ( v_{1},\ldots,v_{n} ) \in ({\mathbb{Z}}/q{\mathbb{Z}})^{n}.\]
Suppose we want to encode a message \(\mu\) that is just a single bit,
let us say \(\mu \in \{ 0,1 \}\). Our ciphertext will be a
...
\subsubsection{Operations on encrypted data}
...
\[\operatorname{Flatten}( \mathbf{x} ) = \mathbf{x}',\] where
...
compute another matrix \(\operatorname{Flatten}(C)\) such that:
...
The \(\operatorname{Flatten}\) process is essentially the same binary-expansion
process we used above to turn \(\mathbf{x}\) into \(\mathbf{x}'\),
applied to each \(k + 1\) entries of each row of the matrix \(C\).

So now, using this \(\operatorname{Flatten}\) operation, we can insist that all
...
\[\operatorname{Flatten}( C_{1}C_{2} ).\]
