...
finite group (for example ${\mathbb{F}}_{p}^{\times}$, or an elliptic curve).
...
\item Bob takes the $i$-th ciphertext $\operatorname{Enc}_{a}( x_{i} )$,
...
\item Alice applies $\operatorname{Dec}_{a}$ to it and sends back to Bob;
  because the encryption scheme is commutative, this gives
  \[ \operatorname{Dec}_a( \operatorname{Enc}_{b}( \operatorname{Enc}_{a}( x_{i} ) )) =
      \operatorname{Enc}_{b}( x_{i} ) \]
