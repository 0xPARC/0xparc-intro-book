...
As we've seen (Section \ref{ec}), a lot of cryptography relies on hard math
...
learning with errors problem (LWE)\footnote{Introduced by Oded Regev in 2009: \url{https://arxiv.org/abs/2401.03703}}. 
The problem is to solve systems of
...
or real numbers, you're solving for integers modulo $q$.

\subsection{A small example of a small LWE problem to try by hand}\label{lwe-small}
...
\begin{example}{A concrete example of an LWE problem}{}
We are working over $\FF_{11}$, and there is some secret vector
...
$S \subset \FF_{11}$, to mean the statement
...
assumption, is one of those ``hard problems that you can build cryptography on.''
The problem is to solve for constants
\[a_{1},\ldots,a_{n} \in \ZZ/q\ZZ,\] given a bunch of
