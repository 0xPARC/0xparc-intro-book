...
``programming language'' that we'll write our function in. For PLONK,
the choice that's used is: \textbf{systems of quadratic equations over
$\FF_{q}$}. In other words, PLONK is going to give us the
ability to prove that we have solutions to a system of quadratic
equations. We state this formally as follows.

\begin{goal}{}{}
  Suppose we have a system of $m$ equations in $k$ variables $x_{1},\ldots,x_{k}$:
  \begin{align*}
    Q_1( x_{1},\ldots,x_{k} ) & = 0 \\
    &\vdots \\
    Q_{m}( x_{1},\ldots,x_{k} ) & = 0.
  \end{align*}
  Of these $k$ variables, the first $\ell$ variables $x_{1}$, \dots, $x_{\ell}$
  have publicly known, fixed values; the remaining $k - \ell$ are unknown.

  Then PLONK will let Peggy prove to Victor the following claim:
  I know $k - \ell$ values $x_{\ell + 1},\ldots,x_{k}$ such that
  (when you combine them with the $\ell$ public fixed values $x_{1},\ldots,x_{\ell}$)
  the $k$ values $x_{1},\ldots,x_{k}$ satisfy all $m$ quadratic equations.
\end{goal}
...
problem into algebra is called \alert{arithmetization}. It turns out that
quadratic equations over $\FF_{q}$, viewed as an NP problem
...
\begin{remark}{Example of Quad-SAT encoding 3-SAT}{np-complete}
\label{np-complete}
...
the goal of projects like Circom\footurl{https://docs.circom.io/},
which gives a high-level language that compiles a function like SHA-256
into a system of equations over $\FF_{q}$ that can be used in
practice. Systems like this are called \alert{arithmetic circuits}, and
Circom is appropriately short for ``circuit compiler''. If you're
curious, you can see how SHA-256 is implemented in Circom on
GitHub.\footurl{https://github.com/iden3/circomlib/blob/master/circuits/sha256/sha256.circom}

So, the first step in proving a claim like ``I have a message $M$ such
that $\sha(M) = \mathtt{0xa91af3ac}\dots$'' is to translate the
claim into a system of quadratic equations.

One approach (suggested by Remark \ref{tcb:np-complete}) is
...
would be represented by 32 public bits $x_{1}$, \dots, $x_{32}$; the
unknown message $M$ would be represented by some private variables;
and the calculation of $\sha$ would introduce a series
...
$\FF_{q}$, which are globally known. The confusing choice of
subscripts stands for ``Left'', ``Right'', ``Output'',
``Multiplication'', and ``Constant'', respectively.

The \alert{copy constraints} are a bunch of assertions that some of the
...
$y = x^{2} + 2$ in PLONK. We'll break this down into two steps:
\[\begin{aligned}
...
We'll assign the variables $a_{i},b_{i},c_{i}$ for these two gates
...
And finally, we'll assign copy constraints to make sure the variables
...
In PLONK, we'll assume that $q \equiv 1 \pmod n$, which
means that we can fix $\omega \in \FF_{q}$ to be a primitive
$n$-th root of unity.

Then, by polynomial interpolation, Peggy chooses polynomials $A(X)$,
$B(X)$, and $C(X)$ in $\FF_{q}[X]$ such that
\begin{equation}
\label{plonk-setup}
...
Algorithm \ref{tcb:root-check} on the set
...
fast Fourier transform. (Note: When you're working in a finite field,
the fast Fourier transform is sometimes called the ``number theoretic
transform'' (NTT) even though it's exactly the same as the usual FFT.)
...
polynomial $Q_{L}(X) \in \FF_{q}[X]$ of degree
$n - 1$ such that
...
application of Algorithm \ref{tcb:root-check}:

\begin{algo}{Gate check}{}
\begin{enumerate}
\item
Both parties interpolate five polynomials
$Q_{\ast} \in \FF_{q}[X]$ from the $5n$
 coefficients $q_{\ast}$ (globally known from the PLONK instance).
\item
Peggy uses Algorithm \ref{tcb:root-check} to convince Victor that
...
is simply an application of Algorithm \ref{tcb:root-check}. Let's say the public
witnesses are the values $a_{i}$, for all $i$ in some set $S$. (If
some of the $b$'s and $c$'s are also public, we'll just do the
...
Now the prover runs Algorithm \ref{tcb:root-check} to prove that
$A - A^{\text{public}}$ vanishes on $S$. (And similarly for $B$
and $C$, if needed.) And we're done.
