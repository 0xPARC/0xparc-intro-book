...
\begin{align*}
  P &\colon \FF_q^m \times \FF_q^m \to \FF_q \\
  P(\vec i, \vec j) &\coloneqq
    {\wt a}_{\vec i, \vec j} \wt A(\vec i) \wt A(\vec j)
    + \frac{1}{|\HH|^m} {\wt b}_{\vec i} \wt A(\vec i)
\end{align*}
in place of what we called $P$ in the sum-check section.

I want to stress now the tilde's above are actually hiding a lot of work.
Let's unpack it a bit: what does ${\wt a}_{\vec i, \vec j}$ mean
when $\vec i \notin \HH^m$?
...
\begin{itemize}
  ...
  this lets us define $\wt a_{\vec i, \vec j}$ for $\vec i$ and $\vec j$
...
Peggy has printed the single-variable polynomial that arises from sum-check.
...
$\wt a_{\vec i, \vec j}$ and $\wt b_{\vec i}$, and finally verifies that
\[
  P(\vec i, \vec j) \coloneqq
  {\wt a}_{\vec i, \vec j} B_0[\vec i] B_0[\vec j]
  + \frac{1}{|\HH|^m} {\wt b}_{\vec i} B_0[\vec i]
\]
is actually true.
